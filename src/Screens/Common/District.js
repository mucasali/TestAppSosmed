...
const DistrictSelect = observer(props => {
  const {districts, districtState, getVillage} = useRegionStore();
  const {fetching} = districtState;
      ...
      <View>
        ...
        <ListModal
          ...
          onSelect={value => {
            onSubmit(value);
            setVisible(false);
            getVillage({id_kecamatan: value.id});
          }}
